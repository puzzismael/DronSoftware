...
public class FrameLogin extends JFrame {
	...
	private class BtnEntrarActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if(isDoctor()){
				JOptionPane.showMessageDialog(contentPane,Messages.getString("FrameLogin.23")+supervisor.getNombre()+Messages.getString("FrameLogin.24")+supervisor.getApellidos()); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private class TextContraseñaActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			Supervisor supervisor=esSupervisor();
			if(isDoctor()){
				JOptionPane.showMessageDialog(contentPane,Messages.getString("FrameLogin.27")+supervisor.getNombre()+Messages.getString("FrameLogin.28")+supervisor.getApellidos()); //$NON-NLS-1$ //$NON-NLS-2$
			//	FramePrincipal ventanaPrincipal = new FramePrincipal(doctor,usuarios);
			//	ventanaPrincipal.setVisible(true);
				dispose();
			}else{
